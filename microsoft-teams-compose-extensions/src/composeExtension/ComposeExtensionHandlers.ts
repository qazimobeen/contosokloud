...
const searchApiUrlFormat = "https://api-aus.myconnectwise.net/v2017_5/apis/3.0/service/tickets?conditions=company/id=19373&orderby=dateEntered desc&pageSize=5";
    ...
    public static getOnQueryHandler(bot: builder.UniversalBot): (event: builder.IEvent, query: teams.ComposeExtensionQuery, callback: (err: Error, result: teams.IComposeExtensionResponse, statusCode: number) => void) => void {
        ...
        ): Promise<void>
        {
            ...
            if (initialRunParameter) {
                request({
                    url: searchApiUrlFormat,
                    headers: {
                        "Content-type":"application/json",
                        "Cache-Control":"no-cache",
                        "Authorization": "Basic S2xvdWRUcmFpbmluZytxcHBWZkFNZlVWMXJaZ0tKOk1vU1RCdURzMG5MRlp5b3A="
                    },
                    method: "GET"
                   }, (error, res, body) => { 
                       let resultsConnectWise: any = JSON.parse(body);
                       let connectwiseCardsAsAttachments = resultsConnectWise.map((resultConnectWise) => {
                           let imageUrl = resultConnectWise.priority._info.image_href;
                           let card: builder.ThumbnailCard = new builder.ThumbnailCard();
                           // check user preference for which type of card to create
                           card.title(resultConnectWise.id)
                               .text(resultConnectWise.summary)
                        ...
                        let response = responseObject.attachments(connectwiseCardsAsAttachments).toResponse();
                        callback(null, response, 200);
                });
